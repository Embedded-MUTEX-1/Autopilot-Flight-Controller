...
void DpsClass::begin(I2cDevice &bus)
{
    begin(bus, DPS__STD_SLAVE_ADDRESS);
}

void DpsClass::begin(I2cDevice &bus, uint8_t slaveAddress)
{
    ...
    delay_milis(50); // startup time of Dps3xx
...
int16_t DpsClass::measureTempOnce(float &result, uint8_t oversamplingRate)
{
    ...
    delay_milis(calcBusyTime(0U, m_tempOsr) / DPS__BUSYTIME_SCALING);
    delay_milis(DPS3xx__BUSYTIME_FAILSAFE);
...
int16_t DpsClass::measurePressureOnce(float &result, uint8_t oversamplingRate)
{
    ...
    // wait until measurement is finished
    delay_milis(calcBusyTime(0U, m_prsOsr) / DPS__BUSYTIME_SCALING);
    delay_milis(DPS3xx__BUSYTIME_FAILSAFE);
...
int16_t DpsClass::readByte(uint8_t regAddress)
{
    ...
    if(m_i2cbus->writeByte(m_slaveAddress, regAddress, false) != 0)
        return DPS__FAIL_UNKNOWN; // if 0 bytes were read successfully
    if(m_i2cbus->readByte(m_slaveAddress, (char *)&regValue, true) != 0)
        return DPS__FAIL_UNKNOWN; // if 0 bytes were read successfully
...
int16_t DpsClass::writeByte(uint8_t regAddress, uint8_t data, uint8_t check)
{
    ...
    buf[0] = (char)regAddress;
    buf[1] = (char)data;

    if(m_i2cbus->writeBytes(m_slaveAddress, buf, 2) != 0) {
        return DPS__FAIL_UNKNOWN;
    }
...
int16_t DpsClass::readBlock(RegBlock_t regBlock, uint8_t *buffer)
{
    ...
    m_i2cbus->writeBytes(m_slaveAddress, (char *)&regBlock.regAddress, 1, false);

    if(m_i2cbus->readBytes(m_slaveAddress, (char *)buffer, regBlock.length) != 0)
        return -1;
