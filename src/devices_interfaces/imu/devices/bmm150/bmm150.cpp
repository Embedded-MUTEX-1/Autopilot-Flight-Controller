...
BMM150::BMM150(I2cDevice* w) {
    this->w = w;
}

int8_t BMM150::init() {
    if (initialize() == BMM150_E_ID_NOT_CONFORM)
        return -1;
    return 0;
}

int8_t BMM150::deinit() {
    return 0;
}

int8_t BMM150::updateAndGetData(struct attitudeData &values) {
    read_mag_data();
    values.magX = raw_mag_data.raw_datax;
    values.magY = raw_mag_data.raw_datay;
    values.magZ = raw_mag_data.raw_dataz;
    return 0;
}
...
int8_t BMM150::initialize(void) {
    ...
    delay_milis(BMM150_START_UP_TIME);
...
void BMM150::soft_reset() {
    ...
    delay_milis(BMM150_SOFT_RESET_DELAY);
}
...
void BMM150::i2c_write(short address, short data) {
    ...
    w->writeBytes(BMM150_I2C_Address, buf, 2);
}

void BMM150::i2c_read(short address, uint8_t* buffer, short length) {
    w->writeBytes(BMM150_I2C_Address, (char *)&address, 1, false);
    w->readBytes(BMM150_I2C_Address, (char *)buffer, length);
}
...
uint8_t BMM150::i2c_read(short address) {
    ...
    w->writeBytes(BMM150_I2C_Address, (char *)&address, 1, false);
    w->readBytes(BMM150_I2C_Address, (char *)&byte, 1);
