...
QuickPID::QuickPID(
  ...
  QuickPID::SetTunings_(Kp, Ki, Kd, pMode, dMode, iAwMode);
}
 ...
 **********************************************************************************/
float QuickPID::Compute(float sp, float fb) {
...
******************************************************************************/
void QuickPID::SetTunings_(float Kp, float Ki, float Kd,
                          pMode pMode = pMode::pOnError,
                          dMode dMode = dMode::dOnMeas,
                          iAwMode iAwMode = iAwMode::iAwCondition) {
...
void QuickPID::SetTunings(float Kp, float Ki, float Kd) {
  SetTunings_(Kp, Ki, Kd, pmode, dmode, iawmode);
}
...
void QuickPID::SetOutputLimits(float Min, float Max) {
  ...
  if (mode != Control::manual) {
    myOutput = constrain_(myOutput, outMin, outMax);
    outputSum = constrain_(outputSum, outMin, outMax);
  }
}
...
void QuickPID::getConfig(float &kp, float &ki, float &kd) {
      kp = this->dispKp;
      ki = this->dispKi;
      kd = this->dispKd;
  }
